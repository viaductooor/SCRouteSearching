...
import org.lab1505.map.Resource;
...
public class CSVNewYorkParser {
	...
	public ArrayList<Resource> parse() {
		parse((resource -> resources.add(resource)));
		return resources;
	}

	public void parse(Consumer<Resource> resourceConsumer){
		try {
...
//					continue;
//				}
				resourceConsumer.accept(new Resource(pickupLat, pickupLon, dropoffLat, dropoffLon, time));
			}
			sc.close();
		} catch (Exception e) {
